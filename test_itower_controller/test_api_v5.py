...
def blm_discovery_start():
    resp = requests.post('http://192.168.20.1/v1/api/slave', headers=httpd_header, data='{"action": "discst","params": {}}' ,timeout=1)
    # print("Response content len:", len(resp.content), '\n')
    assert resp.status_code==200

def blm_discovery_list():
    resp = requests.get('http://192.168.20.1/v1/api/slave/?id=240', timeout=3)
    # print("Response content len:", len(resp.content), '\n')
    assert resp.status_code==200
